...
import ru.tokmakov.item.dto.ItemWithBookingDateDto;
import ru.tokmakov.item.dto.ItemWithCommentsDto;
...
public interface ItemRepository extends JpaRepository<Item, Long> {
    @Query("SELECT new ru.tokmakov.item.dto.ItemDto(i.id, i.name, i.description, i.available, i.requestId) " +
           "FROM Item i " +
    ...
    @Query("SELECT new ru.tokmakov.item.dto.ItemWithBookingDateDto(i.id, i.name, i.description, i.available," +
    ...
    @Query("SELECT new ru.tokmakov.item.dto.ItemWithCommentsDto(i.id, i.name, i.description, i.available, " +
           "(SELECT MIN(b.start) FROM Booking b WHERE b.item.id = i.id AND b.start > CURRENT_TIMESTAMP), " +
           "(SELECT MIN(b.start) FROM Booking b WHERE b.item.id = i.id AND b.start > CURRENT_TIMESTAMP), " +
           "null) " +
           "FROM Item i " +
